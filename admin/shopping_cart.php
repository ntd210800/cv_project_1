	...
	<div class="content-center main-body">
		...
		<?php
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				echo "<td>".$row["name"]."</td>";
				echo "<td style='text-align: center'><img width='50' height='50' src='../public/image/".$row["hinh_anh"]."' alt='Lỗi hiển thị ảnh'></td>";
				// Cài đặt số lượng sản phẩm tối thiểu là 1
				echo "<td class='box-num-count'><input type='number' name='soluong[".$row["ma_sp"]."]' min='1' value=".$arrCart[$row["ma_sp"]]."></td>";
				echo "<td>".formatCurrency($row["gia"])."</td>";
				$totalBill += ($arrCart[$row["ma_sp"]] * $row["gia"]);
				echo "<td>".formatCurrency($arrCart[$row["ma_sp"]] * $row["gia"])."</td>";
				echo "<td style='width: 50px; text-align: center;'><a href='shopping_remove.php?ma_sp=".$row["ma_sp"]."'><img src='../admin/image/trash.png' alt='Xóa'></a></td>";
				echo "</tr>";
			}
		?>
			<tr>
